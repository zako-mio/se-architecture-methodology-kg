# 缺口清单：paid / user-copy 书籍（一次性请求用户提供合法本地副本）
...
> 生成日期：2026-09-10
> 用途：阶段2 仅提取**目录 / 公开章节 / 官方摘要**用于知识结构；**实体全文待用户提供合法本地副本**后方可解析。
> 版权纪律：请提供您**合法拥有**的副本（已购买/授权）；本流程不下载、不传播受版权保护的全文；原文只入本地语料层，公开仓库只承载消化重组后的知识（`00-plan/contract.md §5`）。

## 0. 如何使用本清单

1. 按 **P0 → P1 → P2** 优先级挑选您手头已有（或愿意购置）的书目，提供本地文件（PDF/EPUB/MOBI）或合法访问途径。
2. 提供方式（任选）：
   - 将文件放入任一本地目录，把**绝对路径**告知主Agent；
   - 或告知官方购买后导出的文件位置。
3. 收到副本后：有文本层走 `pdf-worker`、扫描件走 `pdf-ocr` 解析 → 摘要落 `02-research/`；原文只入本地语料层。
4. 若某本暂无副本，不阻塞——其**公开目录/作者博客/官方摘要**可在阶段2 以"结构+链接"形式先行覆盖（下表"免费线索"列）。

> **提示**：阶段2 知识结构所需的最小集是 **P0（14 本）**。P1/P2 可分批。
...
| paid/user-copy（本清单） | 30 | P0=14 / P1=10 / P2=6 |
| 候选总数 | 35 | 与 `D-book-candidates.json` 一致 |

## 5. 说明与边界

- **最小可行集**：P0 的 14 本覆盖 SE 通论、架构、DDD、整洁/重构、数据密集型、DevOps/交付、设计模式七个核心域的权威文本；缺失会显著削弱 S1/S2 层证据密度。
- **可先行的公开层**：即使不提供副本，多数书有作者官网/官方博客/官方模式目录可作为 S2 公开线索（上表"免费线索"列），但**书本体概念原句**仍需本地副本方可一手核验。
- **解析管线**：`pdf-worker`（有文本层）/ `pdf-ocr`（扫描件）；原文仅入本地语料层，不进公开仓库。
- **不阻塞**：本缺口与阶段2 联网主线并行，不阻塞 Wave A/B 的证据包产出。

> 如您希望精简，请优先提供 **P0**；其余可后续增量补充。
